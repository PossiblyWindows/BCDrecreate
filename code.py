...
KEYSYS_CHALLENGE = KEYSYS_BASE + "/challenge"
KEYSYS_VALIDATE = KEYSYS_BASE + "/validate"
KEYSYS_TRIAL = KEYSYS_BASE + "/trial"
KEYSYS_WTIER = KEYSYS_BASE + "/wtier"
KEYSYS_MSG = KEYSYS_BASE + "/msg"
ALLOWED_TIERS = ("Basic", "Basic+", "Pro", "Premium")
CHATGPT5FREE_ENDPOINT = "https://chatgpt5free.com/wp-json/chatgpt-pro/v1/chat"
...
def request_challenge_nonce(user: str, key: str, timeout: float = 8.0) -> Optional[str]:
    """Fetch a fresh nonce from /challenge for HMAC signing."""

    if not user or not key:
        return None
    try:
        res = requests.get(KEYSYS_CHALLENGE, params={"user": user, "key": key}, timeout=timeout)
        js = res.json()
        return js.get("nonce")
    except Exception:
        return None
...
def build_license_signature(key: str, nonce: str, hwid: str) -> str:
    data = f"{nonce or ''}{hwid}".encode("utf-8")
    digest = hmac.new((key or "").encode("utf-8"), data, hashlib.sha256).digest()
    return base64.b64encode(digest).decode("utf-8")


def perform_license_validation(
    user: str, key: str, hwid: str, method: str = "POST", timeout: float = 12.0
) -> Tuple[int, Dict[str, object]]:
    """Validate a license using the nonce + HMAC challenge/response flow."""

    nonce = request_challenge_nonce(user, key, timeout=timeout)
    if not nonce:
        return 503, {"message": "Challenge failed"}
    signature = build_license_signature(key, nonce, hwid)
    payload = {
        "user": user,
        "key": key,
        "hwid": hwid,
    ...
    try:
        if method.upper() == "GET":
            res = requests.get(KEYSYS_VALIDATE, params=payload, timeout=timeout)
        else:
            res = requests.post(KEYSYS_VALIDATE, json=payload, timeout=timeout)
        return res.status_code, res.json()
    except Exception:
        return 503, {"message": "Validation error"}


def request_trial_status(user: str, method: str = "GET", timeout: float = 8.0) -> Dict[str, object]:
    if not user:
        return {"success": False, "message": "missing user"}
    try:
        if method.upper() == "POST":
            res = requests.post(KEYSYS_TRIAL, json={"user": user}, timeout=timeout)
        else:
            res = requests.get(KEYSYS_TRIAL, params={"user": user}, timeout=timeout)
        return res.json()
    except Exception:
        return {"success": False, "message": "trial request failed"}


def update_license_tier(key: str, tier: str, timeout: float = 10.0) -> Dict[str, object]:
    if not key or not tier:
        return {
            "success": False,
            "message": "Missing required fields: key, tier",
            "pad": "...",
        }
    ...
    try:
        res = requests.post(KEYSYS_WTIER, json={"key": key, "tier": tier}, timeout=timeout)
        if res.ok:
            js = res.json()
            if js:
                return js
        ...
        return {"success": False, "message": "Tier update failed", "pad": "..."}


def call_chatgpt5free(prompt: str, timeout: float = 15.0) -> Optional[str]:
    body = {
        "messages": [
            {
                "role": "system",
    ...
    try:
        res = requests.post(CHATGPT5FREE_ENDPOINT, json=body, timeout=timeout)
        js = res.json()
...
TIER_MODEL_MAP = {"Basic+": "gpt-4o-mini", "Pro": "gpt-5-nano", "Premium": "gpt-5.1-nano"}


def send_message_via_tier(tier: str, api_key: Optional[str], message: str) -> Dict[str, object]:
    if not message:
        return {"error": "Missing key or message"}
    if not tier:
        return {"error": "Invalid license key"}
    if tier == "Basic":
        content = call_chatgpt5free(message)
        if content is None:
            return {"error": "Server error"}
        return {"tier": tier, "model": "chatgpt5free-proxy", "response": content}
    model = TIER_MODEL_MAP.get(tier)
    if not model:
        return {"error": "This tier cannot use /msg"}
    if not api_key:
        return {"error": "No API key stored in license"}
    try:
        ...
        return {
            "tier": tier,
            "model": model,
            "response": choice.message.content if choice and choice.message else None,
        }
    except Exception:
        return {"error": "Server error"}
...
class LicenseManager:
    ...
    def validate_and_store(self, ks_user: str, ks_key: str) -> Tuple[bool, str]:
        """Validate via the hardened challenge/response API and persist if valid."""
        ...
        code, js = perform_license_validation(ks_user, ks_key, hwid, method="POST")

        if code == 200 and js.get("valid"):
        ...
        if code == 404:
            ...
            log_message(msg, self.logger)
            return False, msg
        msg = T(self.lang, "lic_error", e=js.get("message", "Validation failed"))
        log_message(msg, self.logger)
        return False, msg
    ...
    def status(self) -> Dict[str, Optional[str]]:
        ...
        if ks_user and ks_key:
            try:
                hwid = ks_hwid or get_hwid()
                code, js = perform_license_validation(ks_user, ks_key, hwid, method="GET")
                if code == 200 and js.get("valid"):
                    if js.get("hwid"):
                        reg_write("LicenseHWID", js["hwid"])
                    if js.get("user"):
                        reg_write("LicenseUser", js["user"])
                ...
                elif (js.get("message") or "").lower().find("mismatch") >= 0:
                    reg_delete("LicenseUser")
                    reg_delete("LicenseKey")
            except Exception:
